...
CGFloat const offset_HeadersStop = 40.0;
CGFloat const offset_B_LabelsHeader = 0.0;
CGFloat const distance_W_LabelsHeader = 35.0;

@implementation ESTravelTimeline
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    else     self.navigationItem.titleView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@""]];//LogoNavigationBar.png"]];
...
- (PFQuery *)queryForTable {
   ...
   // [query whereKey:kESPhotoUserKey equalTo:self.user];
    [query whereKey:kESStoryUserKey equalTo:self.user];

    [query orderByDescending:@"createdAt"];
   // [query includeKey:kESPhotoUserKey];
    [query includeKey:kESStoryUserKey];

    [query whereKeyExists:@"exclusive"];
