...
@interface ESMyTravels () {
    float previousAmount2;
    
}
@end

@implementation ESMyTravels
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    if (!headerView) {
        headerView = [[ESPhotoFooterView alloc] initWithFrame:CGRectMake( 0.0f, 0.0f, self.view.bounds.size.width, 64.0f) buttons:ESPhotoFooterButtonsDefault1];
        headerView.delegate = self;
    ...
    if (attributesForPhoto) {
        ...
        [headerView setExclusiveStatus:[[ESCache sharedCache] exclusiveForPhoto:photo].boolValue];
...
- (PFQuery *)queryForTable {
    ...
    if (![PFUser currentUser]) {
        ...
        [query whereKey:kESUserObjectIdKey equalTo:curUser.objectId];
...
//    NSDate *sevenDaysAgo = [now dateByAddingTimeInterval:-7*24*60*60];
    ...
    [userQuery whereKey:kESUserObjectIdKey equalTo:curUser.objectId];
    
//    PFQuery *photosFromCurrentUsersQuery = [PFQuery queryWithClassName:self.parseClassName];
//    [photosFromCurrentUsersQuery whereKeyDoesNotExist:@"type"];
//    [photosFromCurrentUsersQuery whereKeyExists:kESPhotoPictureKey];
//    [photosFromCurrentUsersQuery whereKey:@"user" matchesQuery:userQuery];
//    [photosFromCurrentUsersQuery whereKey:@"createdAt" greaterThanOrEqualTo:sevenDaysAgo];
//    [photosFromCurrentUsersQuery whereKey:kESStoryPopularPointsKey greaterThan:[NSNumber numberWithInt:POPULARITY_POINTS_MINS ]];
    
//    PFQuery *videosFromCurrentUserQuery = [PFQuery queryWithClassName:self.parseClassName];
//    [videosFromCurrentUserQuery whereKey:@"type" equalTo:kESVideoTypeKey];
//    [videosFromCurrentUserQuery whereKey:@"user" matchesQuery:userQuery];
//    [videosFromCurrentUserQuery whereKey:@"createdAt" greaterThanOrEqualTo:sevenDaysAgo];
//    [videosFromCurrentUserQuery whereKey:kESStoryPopularPointsKey greaterThan:[NSNumber numberWithInt:POPULARITY_POINTS_MINS]];
    
//    PFQuery *query = [PFQuery orQueryWithSubqueries:[NSArray arrayWithObjects:photosFromCurrentUsersQuery,videosFromCurrentUserQuery,nil]];
    PFQuery *query = [PFQuery queryWithClassName:self.parseClassName];
    [query whereKey:@"user" matchesQuery:userQuery];
    [query includeKey:kESPhotoUserKey];
    [query orderByDescending:@"createdAt"];
    [query whereKey:kESPhotoExclusiveKey notEqualTo:[NSNumber numberWithBool:NO]];
//    [query whereKey:@"createdAt" greaterThan:now];

    NSLog(@"parse Class Name12:%@",self.parseClassName);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath object:(PFObject *)object {
   ...
   // static NSString *ExclusiveIdentifier = @"exclusive";
    
    NSLog(@"The exclusive id is %@", CellIdentifier);
...
- (void)photoFooterView:(ESPhotoFooterView *)photoFooterView didTapCommentOnPhotoButton:(UIButton *)button  photo:(PFObject *)photo {
    ...
    if ([photo objectForKey:kESVideoFileKey]) {
        ESMyTravelsVideoDetails *videosDetailsVC = [[ESMyTravelsVideoDetails alloc] initWithPhoto:photo];
        [self.navigationController pushViewController:videosDetailsVC animated:YES];
    }
    else {
        ESMyTravelsPhotoDetails *photosDetailsVC = [[ESMyTravelsPhotoDetails alloc] initWithPhoto:photo];
        [self.navigationController pushViewController:photosDetailsVC animated:YES];
    }
...
//    [photoFooterView shouldReEnableLikeButton:number];
    ...
    BOOL excl = !button.selected;
    [photoFooterView setExclusiveStatus:excl];
    
    [[ESCache sharedCache] setPhoto:photo exclusive:excl];
    
//    if (excl)
    {
        [super loadObjects:0 clear:YES];
    }
}
...
- (void)didTapOnTextPostAction:(UIButton *)sender {
    ...
    if (text) {
        ESMyTravelsPhotoDetails *photosDetailsVC = [[ESMyTravelsPhotoDetails alloc] initWithPhoto:text];
        [self.navigationController pushViewController:photosDetailsVC animated:YES];
